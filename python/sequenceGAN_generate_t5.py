...
def clean_chinese_text(text):
    """徹底清理中文文本"""
    # 保存英文單詞間的空格
    english_words = re.findall(r'[a-zA-Z0-9]+\s+[a-zA-Z0-9]+', text)
    for i, word_pair in enumerate(english_words):
        placeholder = f"_ENGSPACE{i}_"
        word_pair_no_space = word_pair.replace(' ', placeholder)
        text = text.replace(word_pair, word_pair_no_space)
    ...
    for i in range(len(english_words)):
        text = text.replace(f"_ENGSPACE{i}_", ' ')
...
def main(batch_size=1, input_file=None):
    ...
    if input_file and input_file.endswith('.csv'):
        ...
        for post, responses in discussions.items():
            ...
            # 徹底清理中文文本
            cleaned_texts = [clean_chinese_text(text) for text in processed_texts]
